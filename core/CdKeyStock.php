...
final class BTL_CdKey_Stock
{
    ...
    public static function cachedAvailableCount(int $productId, int $variationId): int
    {
        ...
        return (int)BTL_Cache::remember(
            "stock_count_{$productId}_{$variationId}",
            static function () use ($productId, $variationId) {
                return self::availableCount($productId, $variationId);
            },
    ...
    private static function invalidate_stock_display_cache(): void
    {
        BTL_Cache::flushGroup(self::STOCK_DISPLAY_CACHE_GROUP);
    }

    public static function assignNext(
        int $productId,
        int $variationId,
        int $orderId,
        int $itemId
        ...
        if ($plaintext === null) {
            BTL_Helpers::logger(
                "CdKeyStock: decrypt failed for stock row {$row->id}"
            );

            return false;
        }

        BTL_Secure_Fields::store(
            $orderId,
            $itemId,
            'cdkey',
            $plaintext
    ...
    public static function maybe_assign_on_status_change(
        $orderId,
        $oldStatus,
        $newStatus,
        $order
    ): void {
        if (!in_array($newStatus, self::AUTO_ASSIGN_STATUSES, true)) {
            return;
        }
        ...
        foreach ($order->get_items() as $itemId => $item) {
            ...
            $already = BTL_Secure_Fields::countByOrderItem(
                (int)$orderId,
                (int)$itemId,
                'cdkey'
            );
    ...
    public static function backfillPendingOrders(
        int $productId,
        int $variationId
    ): void {
        global $wpdb;
        ...
        $itemIds = $wpdb->get_col($wpdb->prepare(
            "SELECT DISTINCT oi.order_item_id
             ...
             INNER JOIN {$wpdb->prefix}woocommerce_order_itemmeta oim_vid
               ...
               AND oim_vid.meta_value = %d
             INNER JOIN {$wpdb->prefix}woocommerce_order_itemmeta oim_delivery
                ON oim_delivery.order_item_id = oi.order_item_id
               AND oim_delivery.meta_key = 'روش تحویل'
               AND oim_delivery.meta_value = 'code'
            ...
            if ($nowAssigned >= $needed) {
                if ($order) {
                    $order->add_order_note(
                        '✅ کد سی‌دی‌کی به‌صورت خودکار پس از افزودن موجودی جدید تخصیص یافت.'
                    );
                }
    ...
    private static function notify_staff_low_stock(
        int $orderId,
        int $itemId,
        string $productName,
        int $assigned,
        ...
        foreach (self::staffUserIds() as $staffId) {
            BTL_Notifications::push(
                ...
                sprintf(
                    'سفارش #%d — «%s»: فقط %d از %d کد موجود بود.',
                    $orderId,
                    $productName,
                    $assigned,
                    $needed
    ...
    public static function register(): void
    {
        register_graphql_field('OptimizedVariationItem', 'codeStockCount', [
            ...
            'resolve' => static function ($variation) {
                ...
                $productId = (int)wp_get_post_parent_id($variationId);

                if (!$productId) {
                    return 0;
                }

                return BTL_CdKey_Stock::cachedAvailableCount(
                    $productId,
                    $variationId
                );
            },
        ...
        register_graphql_field('LineItem', 'cdkeyReady', [
            ...
            'resolve' => static function ($item) {
                ...
                return BTL_Secure_Fields::countByOrderItem(
                    $orderId,
                    $itemId,
                    'cdkey'
                ) >= $needed;
